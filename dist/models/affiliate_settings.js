...
async function createAffiliateSettingsTable() {
  ...
  `);
    ...
    const expected = {
        ...
        percent: "ALTER TABLE affiliate_settings ADD COLUMN percent INTEGER NOT NULL",
        mode: "ALTER TABLE affiliate_settings ADD COLUMN mode TEXT NOT NULL CHECK (mode IN ('fixed', 'percentage')) DEFAULT 'percentage'",
        panel_id: "ALTER TABLE affiliate_settings ADD COLUMN panel_id INTEGER NOT NULL REFERENCES panels(panel_id) ON DELETE CASCADE",
    };
...
exports.default = createAffiliateSettingsTable;
