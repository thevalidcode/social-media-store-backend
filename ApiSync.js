...
/* ------------------------------------------------------------------
 *  NUMERIC SAFETY HELPERS  (shared across all files)
 * ------------------------------------------------------------------ */
...
const sendRefillToMainServer = async (orderId, panel_id) => {
  try {
    ...
    /* 4. store refill row & kick status check --------------------- */
...
const getOrderDetailsFromMainServer = async (orderData, panel_id) => {
  try {
    ...
    /* fetch services list once if needed --------------------------- */
    let services;
...
/* ------------------------------------------------------------------
 *  SERVICE‑LEVEL SYNC (incremental update of existing rows)
 * ------------------------------------------------------------------ */
const updateServices = async () => {
  try {
    ...
    for (const panel_id of panelIds) {
      ...
      /* cache provider API responses to avoid N×M calls ------------- */
      const provCache = {}; // { url: { balance, services } }
      ...
      for (const svc of services) {
        ...
        const liveSvc = list.find(
          (x) => String(x.service) === String(svc.provider_id)
        );
...
const syncServices = async () => {
  try {
    ...
    for (const p of panels) {
      ...
      /* pull once per panel to avoid repeated I/O ------------------- */
      const services = await getDocs("services", panel_id);
      ...
      for (const prov of providers) {
        ...
        for (const s of svcList) {
          ...
          if (exists) continue; // we only INSERT here; updates are handled by updateServices
