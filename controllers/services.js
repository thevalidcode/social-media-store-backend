const { getDocs, addPanelDoc, updatePanelDoc } = require("../crud");
const { v4: uuidv4 } = require("uuid");
const axios = require("axios");
const { checkAdminApiKey, checkKey } = require("../utils/checkapikey");
...
exports.importServices = async (req, res) => {
  ...
  if (!selectedServices || !panel_id || !key) {
  ...
  try {
    ...
    for (const selId of selectedServices) {
      maxId++;
      categoryId++;
...
exports.updateCatPosition = async (req, res) => {
  const { data, panel_id, key } = req.body;
